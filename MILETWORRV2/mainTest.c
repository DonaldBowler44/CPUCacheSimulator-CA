...
//     // Check if the tag already exists in the inner queue for the cache block with the current index
//     Cache_block* current = *outerQueue;
//     while (current != NULL) {
//         if (current->index == indexToEnqueue) {
//             Block_Info* blockInfo = current->block_info;
//             Block_Info* oldestBlockInfo = blockInfo; // Initialize the pointer to the oldest element

//             // Find the oldest tag in the inner queue
//             while (blockInfo != NULL) {
//                 if (blockInfo->next == NULL) {
//                     oldestBlockInfo = blockInfo;
//                 }
//                 blockInfo = blockInfo->next;
//             }

//             // Check if the tag already exists in the inner queue
//             blockInfo = current->block_info;
//             while (blockInfo != NULL) {
//                 if (blockInfo->tag == tagToEnqueue) {
//                     printf("Tag %x is already in the inner queue for index: %x. Rejecting it!\n", tagToEnqueue, indexToEnqueue);
//                     return;  // Reject the duplicate tag
//                 }
//                 blockInfo = blockInfo->next;
//             }

//             // If the tag is not found in the inner queue, enqueue it
//             if (!checkIfCapacity(current, indexToEnqueue, associativity)) {
//                 if (oldestBlockInfo != NULL) {
//                     // Replace the oldest tag with the new tag
//                     oldestBlockInfo->tag = tagToEnqueue;
//                     printf("Tag %x has replaced the oldest tag in the inner queue for index: %x\n", tagToEnqueue, indexToEnqueue);
//                     return;  // Exit the function after replacement
//                 } else {
//                     // This should not happen if checkIfCapacity is implemented correctly
//                     printf("Error: Inner queue capacity reached, but cannot find the oldest tag.\n");
//                     return;
//                 }
//             }
//         }
...
int main(int argc, char* argv[])
{
    ...
    Queue* queue = initializeQueue();
        ...
        while (!isEmpty(queue)){
            ...
            double Waste = cache->cost / OneKB * UnusedKB;
            int unused_cache_blocks = cache->num_blocks - cache->TotalCacheAccesses;
            ...
            printf("Miss Rate: %.4f%%\n", missRate);
            printf("CPI: %.2f\n", CPI);
            printf("Unused Cache Space: %.2f KB / %.2f KB = %.2f%%\n", UnusedKB, (float)cache->impMeminKB, (UnusedKB / (float)cache->impMeminKB) * 100);
